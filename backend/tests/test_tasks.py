...
class TestPlaylistTransferLogic:
    ...
    def test_youtube_to_spotify_core_logic(self, mock_spotify_client_class, mock_youtube_client_class):
        ...
        for song in songs:
            try:
                search_result = spotify_client.search_song(f"{song.artist} {song.track}")
                if search_result:
                    added = spotify_client.add_song_to_playlist('target_playlist', search_result['id'])
                    if added:
