...
class SpotifyClient:
    ...
    def get_tracks_from_playlist(self, playlist_id: str) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                if response.status_code == 200:
                    ...
                    for item in items:
                        ...
                        if track and track.get("name"):
                            ...
                            tracks.append(
                                {"track": track["name"], "artist": ", ".join(artists)}
                            )
            ...
            logger.info(
                f"Retrieved {len(tracks)} tracks (max 15) from playlist {playlist_id}"
            )
            return tracks
            ...
            if response.status_code == 200:
                ...
                if tracks:
                    ...
                    return {
                        ...
                        "artwork_url": track_data.get("album", {})
                        .get("images", [{}])[0]
                        .get("url"),
                    }
                else:
    ...
    def add_song_to_playlist(self, song_data: Dict[str, Any], playlist_id: str) -> bool:
        ...
        try:
            ...
            data = {"uris": [f"spotify:track:{song_data['id']}"]}
            ...
            if response.status_code in [200, 201]:
                logger.info(
                    f"Successfully added '{song_data['name']}' to playlist {playlist_id}"
                )
                return True
            else:
                logger.error(
                    f"Failed to add song to playlist: {response.status_code} - {response.text}"
                )
                return False
    ...
    def add_song_to_spotify(self, song_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            data = {"ids": [song_data["id"]]}

            response = requests.put(
                f"{self.base_url}/me/tracks", headers=headers, json=data
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to add song to liked songs: {response.status_code} - {response.text}"
                )
                return False
